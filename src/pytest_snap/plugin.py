...
def pytest_addoption(parser):  # pragma: no cover - exercised via integration
	# Keep legacy option group name/prefix for now to avoid breaking user CI configs.
	group = parser.getgroup("html-baseline")
	def add_unique(*names, **kwargs):  # safe re-registration wrapper
		try:  # pragma: no cover - defensive
			group.addoption(*names, **kwargs)
		except ValueError:
			# Option already registered (likely due to legacy plugin also loaded); ignore.
			pass
	# New preferred --snap-* aliases listed first; legacy --html-* kept for compatibility.
	add_unique("--snap-save-baseline", "--html-save-baseline", action="store", dest="html_save_baseline")
	add_unique("--snap-baseline", "--html-baseline", action="store", dest="html_baseline")
	add_unique("--snap-slower-threshold-ratio", "--html-slower-threshold-ratio", action="store", dest="html_slower_threshold_ratio", type=float, default=1.30)
	add_unique("--snap-slower-threshold-abs", "--html-slower-threshold-abs", action="store", dest="html_slower_threshold_abs", type=float, default=0.20)
	add_unique("--snap-min-count", "--html-min-count", action="store", dest="html_min_count", type=int, default=0)
	add_unique(
		"--snap-fail-on", "--html-fail-on",
		action="store",
		dest="html_fail_on",
		choices=["new-failures", "slower", "budgets", "any"],
		default="new-failures",
	)
	add_unique("--snap-diff-json", "--html-diff-json", action="store", dest="html_diff_json")
	add_unique(
		"--snap-verbose", "--html-baseline-verbose",
		action="store_true",
	...
	add_unique(
		"--snap-badges", "--html-baseline-badges",
		action="store_true",
	...
	add_unique(
		"--snap-flake-threshold", "--html-flake-threshold",
		action="store",
	...
	add_unique(
		"--snap-budgets", "--html-budgets",
		action="store",
		dest="html_budgets",
		help="YAML/JSON file containing performance budgets",
	)
	add_unique(
		"--snap-history-path", "--html-history-path",
		action="store",
	...
	add_unique(
		"--snap-history-max", "--html-history-max",
		action="store",
...
def pytest_configure(config):  # pragma: no cover - integration
	...
	try:
		args = list(getattr(config.invocation_params, "args", []) or [])
		if any(a.startswith("--html-") for a in args) and not any(a.startswith("--snap-") for a in args):
			print("[pytest-snap] WARNING: --html-* option names are deprecated; use --snap-* equivalents.")
...
def pytest_sessionstart(session):  # pragma: no cover - integration
	...
	# Track seen test ids to avoid duplicate recording if legacy + new plugin both loaded
	setattr(session.config, '_snap_seen_ids', set())
	# Rationale: during rename period both entry points 'html_baseline' and 'snap' can load the same
	# implementation twice. We deduplicate at record time rather than relying solely on guarding
	# option registration to ensure snapshot JSON isn't polluted with duplicates.
...
def pytest_runtest_makereport(item, call):  # pragma: no cover - integration
	...
	# Deduplicate if hook executed twice via both entry points
	seen = getattr(config, '_snap_seen_ids', None)
	if isinstance(seen, set):
		if rec.id in seen:
			# Remove just-added duplicate
			config._html_baseline_records.pop()  # type: ignore[attr-defined]
			return
		seen.add(rec.id)
